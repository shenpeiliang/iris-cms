!function (win) {
    ...
    Xadmin.prototype.init = function () {
        ...
        element.tabChange('xbs_tab', layui.data('tab_list_active').tab_list_active_key || 'home_lay_id_tag');
    };
    /**
     * [end 执行结束要做的]
    ...
    Xadmin.prototype.end = function () {
        var ativeValue = layui.data('tab_list_active').tab_list_active_key;

        $('.J_NavItem').each(function (k, item) {
            var urlMd5 = md5($(item).data('url'));
            if (ativeValue == urlMd5) {
                $(item).addClass('active').parents('.sub-menu').show();
                $(item).parents('.sub-menu').find('.nav_right').html('&#xe6a6;');
                $(item).parents('.sub-menu').siblings('a').find('.nav_right').html('&#xe6a6;');
            }
        });
    };
...
layui.config({
    ...
    format_time: '/static/js/mod/format_time',
    ie_version: '/static/js/mod/ie_version',
    utils: '/static/js/mod/utils',
});
...
layui.use(['layer', 'element', 'form'], function () {
    ...
    form.on("checkbox(f_checkall)", function (data) {
        $('.J_FCheckbox').prop('checked', data.elem.checked);
        form.render('checkbox');
    });

    var admin = window.admin = {};
    var misAjaxLoadingIndex = '';

    admin.ajax = function (opts) {
        misAjaxLoadingIndex = layer.open({type: 3});
        ...
        $.ajax({
            ...
            layer.close(misAjaxLoadingIndex);
            if (ret.data == 'NOT_LOGIN') {
                window.parent.parent.parent.location.href = '/login';
                return;
            }
    ...
    $('#J_NewWidown').on('click', function () {
        var url = location.href;
        if(!url) {
            url = $(window.parent.document.querySelectorAll('.layui-tab-item.layui-show')).find('iframe').attr('src');
        }
        url && window.open(url);
    });

    var navItems = window.parent.document.querySelectorAll('.J_NavItem');

    var closeAllLeftNav = function () {
        $(navItems).parents('.sub-menu').find('.nav_right').html('&#xe697;');
        $(navItems).parents('.sub-menu').siblings('a').find('.nav_right').html('&#xe697;');
    ...
    element.on('tab(xbs_tab)', function (data) {
        var layidkey = $('#J_TabTitle li').eq(data.index).attr('lay-id');

        layui.data('tab_list_active', {
            key: 'tab_list_active_key', value: layidkey
        });

        $.each(navItems, function (k, item) {
            //首页
            if (layidkey == 'home_lay_id_tag') {
                closeAllLeftNav();
            } else {
                if (md5($(item).data('url')) == layidkey) {
                    //恢复所有导航箭头向上
                    closeAllLeftNav();
                    ...
                    $(item).addClass('active').parents('.sub-menu').show();
                    $(item).closest('.sub-menu').siblings('a');
                }
            }
        });
    });
